...
def test_window_defaults_match_processing_defaults(app, settings) -> None:
    ...
    assert window.form.labelForField(window.play_count_combo).text() == "播放時間"
    assert [window.play_count_combo.itemText(index) for index in range(4)] == ["1 秒", "2 秒", "3 秒", "4 秒"]
...
def test_platform_switch_immediately_applies_line_playback_semantics(app, settings, tmp_path) -> None:
...
def test_group_list_shows_post_export_status_reason_and_invalidates_on_change(app, settings, tmp_path) -> None:
    ...
    job.post_export_validation = ExportValidationResult("warning", ("line_size_warning:982341",), 982_341, 1, 2000)
    ...
    for change in (
        ...
        job.post_export_validation = ExportValidationResult("ok", ("line_export_ok:900000:2:2000",), 900_000, 2, 2000)
        change()
        assert job.post_export_validation is None
    job.post_export_validation = ExportValidationResult("ok", ("line_export_ok:900000:2:2000",), 900_000, 2, 2000)
...
def test_language_switch_is_live_and_persisted(app, settings) -> None:
    ...
    assert window.form.labelForField(window.play_count_combo).text() == "播放时间"
    assert [window.play_count_combo.itemText(index) for index in range(4)] == ["1 秒", "2 秒", "3 秒", "4 秒"]
    window.close()
    ...
    assert restored.form.labelForField(restored.play_count_combo).text() == "Playback Time"
    assert [restored.play_count_combo.itemText(index) for index in range(4)] == [
