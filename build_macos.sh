...
dmg_path="$project_dir/dist/Sticker-Motion-Toolkit-v1.3.0-macOS-arm64.dmg"
mkdir -p "$dmg_root"
ditto "$app_path" "$dmg_root/Sticker Motion Toolkit.app"
ln -s /Applications "$dmg_root/Applications"
hdiutil create -volname "Sticker Motion Toolkit v1.3.0" -srcfolder "$dmg_root" -ov -format UDZO "$dmg_path"
